...
import { Bell, CheckCircle2, CreditCard, Database, KeyRound, Languages, LogOut, Mail, ShieldCheck, Sparkles, UserRound, UserRoundPlus, UsersRound } from "lucide-react";
...
import { CheckoutStatus } from "@/components/paddle/checkout-status";
import { changeEmail, changePassword, inviteWorkspaceMember, removeWorkspaceMember, updatePreferences, updateProfile, updateWorkspaceMember } from "./actions";
...
const MESSAGES: Record<string, string> = {
  ...
  "team-invite": "Team member invited. Existing users can access the workspace now; new users join after sign-up.",
  "team-role": "Team role updated.",
...
const ERRORS: Record<string, string> = {
  ...
  "team-invite": "Enter a valid colleague email and role.",
  "team-plan": "An active Team subscription is required to manage the workspace.",
  "team-seats": "All five Team seats are in use. Remove a member before inviting another.",
  "team-save": "The team change could not be saved.",
};
...
export default async function AccountPage({ searchParams }: {
  searchParams: Promise<{ message?: string; error?: string; checkout?: string }>;
}) {
  ...
  const [{ data: profile }, { data: aal }, usage] = await Promise.all([
    supabase.from("profiles").select("*").eq("id", user.id).maybeSingle(),
    supabase.auth.mfa.getAuthenticatorAssuranceLevel(),
    getUsageContext(user.id),
  ]);
  ...
  const { data: ownWorkspace } = entitlement.isTeam && entitlement.isWorkspaceOwner
    ? await supabase.from("team_workspaces").select("id, name, seat_limit").eq("owner", user.id).maybeSingle()
    : { data: null };
  const { data: ownMembership } = entitlement.isTeam && !entitlement.isWorkspaceOwner
    ? await supabase.from("team_members").select("workspace_id").eq("member_id", user.id).eq("status", "active").maybeSingle()
    : { data: null };
  const { data: memberWorkspace } = ownMembership
    ? await supabase.from("team_workspaces").select("id, name, seat_limit").eq("id", ownMembership.workspace_id).maybeSingle()
    : { data: null };
  const workspace = ownWorkspace ?? memberWorkspace;
  const { data: teamMembers } = workspace
    ? await supabase.from("team_members").select("id, email, display_name, role, status").eq("workspace_id", workspace.id).neq("status", "removed").order("created_at")
    : { data: [] };
  ...
  return (
    <div data-wide className="space-y-6">
      ...
      {params.checkout === "success" && <CheckoutStatus isPaid={entitlement.isPaid} plan={entitlement.plan} />}
      ...
      <section className="flex flex-col justify-between gap-4 rounded-2xl border border-border bg-card p-5 sm:flex-row sm:items-center sm:p-6">
            ...
            {entitlement.isPaid
              ? cancelsOn
                ...
                : renewsOn
                  ...
                  : `${plan === "team" ? "Team" : "Pro"} plan active.`
              : "You're on Free. Upgrade for higher limits, watermark-free output, automation or a shared team workspace."}
          </p></div></div>
        ...
        <div className="shrink-0">
          {entitlement.hasBillingAccount
            ? <ManageBillingButton />
            : entitlement.isTeam ? <span className="text-xs font-medium text-muted-foreground">Billing is managed by the workspace owner.</span>
            : <Button render={<Link href="/pricing" />} size="sm"><Sparkles className="size-4" aria-hidden /> Upgrade plan</Button>}
        </div>
      </section>

      {entitlement.isTeam ? <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
        <div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-secondary text-primary"><UsersRound className="size-5" aria-hidden /></span><div><h2 className="text-lg font-bold text-primary">Team workspace</h2><p className="text-xs text-muted-foreground">Five seats share shipments, assignments, review decisions and approval gates.</p></div></div>
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
          <div><div className="flex items-center justify-between"><p className="text-sm font-bold">{workspace?.name ?? "Team workspace"}</p><span className="text-xs text-muted-foreground">{1 + (teamMembers?.length ?? 0)} / {workspace?.seat_limit ?? 5} seats</span></div><ul className="mt-3 space-y-2"><li className="rounded-xl border border-border bg-background p-3"><p className="text-sm font-semibold">Workspace owner</p><p className="text-xs text-muted-foreground">Owner · billing administrator</p></li>{(teamMembers ?? []).map((member) => <li key={member.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-background p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{member.display_name || member.email}</p><p className="text-xs capitalize text-muted-foreground">{member.role} · {member.status}</p></div>{entitlement.isWorkspaceOwner && <><form action={updateWorkspaceMember} className="flex gap-2"><input type="hidden" name="memberId" value={member.id}/><select name="role" defaultValue={member.role} className="h-9 rounded-lg border border-border bg-card px-2 text-xs"><option value="editor">Editor</option><option value="reviewer">Reviewer</option><option value="approver">Approver</option></select><Button size="sm" variant="outline">Save</Button></form><form action={removeWorkspaceMember}><input type="hidden" name="memberId" value={member.id}/><Button size="sm" variant="outline">Remove</Button></form></>}</li>)}</ul></div>
          {entitlement.isWorkspaceOwner ? <form action={inviteWorkspaceMember} className="rounded-2xl border border-dashed border-border bg-background p-4"><p className="flex items-center gap-2 text-sm font-bold"><UserRoundPlus className="size-4 text-signal" aria-hidden /> Invite team member</p><p className="mt-1 text-xs text-muted-foreground">Members automatically join every shipment in this workspace.</p><input name="email" type="email" required placeholder="colleague@company.com" className={`${input} mt-4`} /><select name="role" className={`${input} mt-3`}><option value="editor">Editor — configure and review</option><option value="reviewer">Reviewer — review and comment</option><option value="approver">Approver — review and approve exports</option></select><Button className="mt-3 w-full" disabled={(teamMembers?.length ?? 0) >= 4}>Invite member</Button></form> : <div className="rounded-2xl border border-border bg-background p-4 text-sm text-muted-foreground">You are a member of this workspace. The owner manages seats, roles and billing.</div>}
        </div>
      </section> : <section className="flex flex-col justify-between gap-4 rounded-2xl border border-border bg-card p-5 sm:flex-row sm:items-center sm:p-6"><div><h2 className="flex items-center gap-2 text-lg font-bold text-primary"><UsersRound className="size-5" aria-hidden /> Team workspace</h2><p className="mt-1 text-sm text-muted-foreground">Shared shipments, five seats, role-based review, assignments and export approvals are available on Team.</p></div><Button render={<Link href="/pricing" />} variant="outline">Compare Team</Button></section>}
