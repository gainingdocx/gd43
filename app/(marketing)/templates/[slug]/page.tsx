...
export default async function TemplatePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section-edge bg-white"><div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:py-16"><BreadcrumbBar><Link href="/">Home</Link><ChevronRight className="size-3" aria-hidden /><Link href="/templates">Templates</Link><ChevronRight className="size-3" aria-hidden /><span>{template.name}</span></BreadcrumbBar><div className="mt-8 flex items-center gap-2"><FreightModeTag mode={templateMode(template.slug)} /><p className="text-sm font-bold uppercase tracking-[0.16em] text-signal">Fillable shipping document</p></div><h1 className="mt-3 text-4xl font-extrabold tracking-tight text-brand-deep sm:text-5xl">{seo?.h1 ?? `Free ${template.name} Template`}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">{seo?.intro ?? template.description}</p><div className="mt-6 flex max-w-3xl gap-3 rounded-2xl bg-secondary p-4 text-sm leading-6 text-secondary-foreground"><FileCheck2 className="mt-0.5 size-5 shrink-0" aria-hidden /><p>{template.purpose}</p></div></div></section>
