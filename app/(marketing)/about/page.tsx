...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "About GainingDocx — Easy PaperWork",
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden border-b border-border bg-white">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_.8fr] lg:py-24">
          <div>
            ...
            <h1 className="mt-4 max-w-4xl text-4xl font-black leading-tight tracking-[-0.04em] text-primary sm:text-5xl">Shipping paperwork should create momentum, not more paperwork.</h1>
          ...
          <div className="grid grid-cols-2 gap-3 rounded-[2rem] border border-border bg-background p-4 shadow-xl">
            {[
              ...
              return <div key={String(label)} className="rounded-2xl bg-white p-5"><IconComponent className="size-6 text-signal" aria-hidden /><p className="mt-4 text-3xl font-black text-primary">{String(value).padStart(2, "0")}</p><p className="text-sm font-semibold text-muted-foreground">{String(label)}</p></div>;
            })}
          </div>
        </div>
      </section>
      ...
      <section id="inside" className="mx-auto max-w-6xl scroll-mt-24 px-4 py-16 sm:px-6 lg:py-24">
        <div className="max-w-3xl"><p className="text-sm font-bold uppercase tracking-[0.16em] text-signal">The information journey</p><h2 className="mt-3 text-3xl font-black tracking-tight text-primary sm:text-4xl">From page image to usable shipment record.</h2><p className="mt-4 leading-7 text-muted-foreground">The workflow keeps the human reviewer in control while automating the mechanical work around them.</p></div>
        <ol className="relative mt-12 grid gap-5 lg:grid-cols-4">
          ...
          {journey.map(({ icon: Icon, title, text }, index) => <li key={title} className="relative rounded-3xl border border-border bg-white p-6 shadow-sm"><span className="relative flex size-14 items-center justify-center rounded-2xl bg-primary text-white ring-8 ring-background"><Icon className="size-6" aria-hidden /></span><p className="mt-6 text-xs font-black uppercase tracking-widest text-signal">0{index + 1}</p><h3 className="mt-2 text-xl font-black text-primary">{title}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{text}</p></li>)}
        </ol>
      </section>

      <section className="border-y border-border bg-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[.8fr_1.2fr] lg:py-24">
          <div><p className="text-sm font-bold uppercase tracking-[0.16em] text-signal">What is inside</p><h2 className="mt-3 text-3xl font-black tracking-tight text-primary sm:text-4xl">A connected toolkit, not a single-purpose upload box.</h2><p className="mt-4 leading-7 text-muted-foreground">Each public resource explains its scope and links into the next useful step. That makes the site useful whether you need automation, a quick calculation, a blank form or a clear explanation.</p></div>
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { icon: FileSearch, title: `${PARSER_PAGES.length} dedicated parsers`, text: "Bills of Lading, invoices, packing lists, waybills, notices, bookings, purchase orders, freight invoices and goods receipts.", href: "/#explore" },
            ...
            ].map(({ icon: Icon, title, text, href }) => <Link key={title} href={href} className="group rounded-2xl border border-border bg-background p-5 transition hover:-translate-y-0.5 hover:bg-white hover:shadow-lg"><Icon className="size-6 text-primary" aria-hidden /><h3 className="mt-4 font-black text-primary">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-primary">Explore <ArrowRight className="size-4 transition group-hover:translate-x-1" aria-hidden /></span></Link>)}
          </div>
        </div>
      </section>

      <section className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24">
        <div className="rounded-[2rem] bg-primary p-7 text-white sm:p-10"><Users className="size-9" aria-hidden /><p className="mt-6 text-sm font-bold uppercase tracking-[0.16em] text-white/60">Who it is for</p><h2 className="mt-3 text-3xl font-black tracking-tight">People who need the facts inside a shipment document.</h2><ul className="mt-7 space-y-4">{audiences.map((item) => <li key={item} className="flex gap-3 text-sm leading-6 text-white/80"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-[#f4c400]" aria-hidden />{item}</li>)}</ul></div>
        <div className="px-0 py-3 sm:px-4"><p className="text-sm font-bold uppercase tracking-[0.16em] text-signal">How we build trust</p><h2 className="mt-3 text-3xl font-black tracking-tight text-primary">AI reads. Code checks. People decide.</h2><div className="mt-7 space-y-5">{[
        ...
        ].map(([title, text]) => <div key={title} className="border-l-4 border-[#f4c400] pl-5"><h3 className="font-black text-primary">{title}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{text}</p></div>)}</div></div>
      </section>

      <section className="border-t border-border bg-white px-4 py-16 sm:px-6">
        <div className="mx-auto max-w-3xl text-center"><p className="text-sm font-bold uppercase tracking-[0.16em] text-signal">Easy PaperWork starts here</p><h2 className="mt-3 text-3xl font-black tracking-tight text-primary sm:text-4xl">Bring one shipping document. Leave with structured data.</h2><p className="mx-auto mt-4 max-w-2xl text-muted-foreground">Try one complete document daily as a guest, review every field, then export the result when it is ready.</p><Button render={<Link href="/app/scan" />} size="lg" className="mt-7 bg-signal text-white hover:bg-signal/90">Parse a document now <ArrowRight aria-hidden /></Button></div>
      </section>
    </>
  );
}
