...
export default async function FeaturePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="mx-auto max-w-4xl px-4 py-14 sm:px-6 lg:py-20"><h2 className="text-3xl font-extrabold text-brand-deep">{headings[2]}</h2><div className="mt-7 divide-y divide-border">{feature.faqs.map((faq) => <details key={faq.q} className="group py-5"><summary className="cursor-pointer list-none font-bold text-primary">{faq.q}</summary><p className="mt-3 leading-7 text-muted-foreground">{faq.a}</p></details>)}</div></section>
  </>;
}
