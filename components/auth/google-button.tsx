...
export function GoogleButton({ next = "/app", label = "Continue with Google" }: { next?: string; label?: string }) {
  ...
  async function continueWithGoogle() {
    ...
    const { error: oauthError } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${location.origin}/auth/callback?next=${encodeURIComponent(safeNext)}`,
        queryParams: { prompt: "select_account" },
      },
    });
    if (oauthError) {
      setError("Google sign-in could not start. Try email and password, or try again.");
      setBusy(false);
    }
  }
  ...
  return (
    <>
      <Button type="button" size="lg" variant="outline" className="h-12 w-full bg-white text-base" disabled={busy} onClick={() => void continueWithGoogle()}>
        {busy ? <Loader2 className="size-5 animate-spin" aria-hidden /> : <GoogleLogo />}
